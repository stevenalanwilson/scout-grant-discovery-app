...
function ProgressBar({ value, max }: { value: number; max: number }): React.ReactElement {
  ...
  return (
    <div
      style={{ background: 'var(--color-surface)', borderRadius: 4, height: 8, overflow: 'hidden' }}
    >
      <div
        style={{
          width: `${pct}%`,
...
export default function ScrapingDashboard(): React.ReactElement {
  ...
  const { history, sources, isLoading, historyError, sourcesError } =
    useScrapingDashboard(isRunning);
        ...
        <>
          ...
          <section aria-label="Summary metrics" style={{ marginBottom: '2rem' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
              ...
              <MetricCard
                label="Grants found (last run)"
                value={lastRun?.grantsFoundCount ?? '—'}
              />
